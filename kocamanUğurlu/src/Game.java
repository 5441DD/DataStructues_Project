...
public class Game extends javax.swing.JFrame {
    ...
    private void act(){
        switch (selectedCell.action) {
                    case 0 -> { resultlbl.setText(resultlbl.getText()+"\n--------------\n"+"<Nothing happened>");}
                    case 1 -> {score+=10; resultlbl.setText(resultlbl.getText()+"\n--------------\n"+"Score increased ( +10 )");
                    scorelbl.setText("Score: "+score);}
                    case 2 -> {score-=5; resultlbl.setText(resultlbl.getText()+"\n--------------\n"+"Score decreased ( -5 )");
                    scorelbl.setText("Score: "+score);}
                    case 3 -> {
                        int move=(int) (Math.random()*6+1);
                        if (selectedCell.cellnum+move>=31) {
                             resultlbl.setText(resultlbl.getText()+"\n--------------\n"+selectedCell.cellnum+"-> FINISH"
                                 + "\n< Moved "+(31-selectedCell.cellnum)+" cells forward >");
                        selectedCell=cellList.get(31);
                            
                            
                        }else if(selectedCell.cellnum==0){
                            resultlbl.setText(resultlbl.getText()+"\n--------------\n"+"START->"+selectedCell.cellnum+move
                                 + "\n< Moved "+move+" cells forward >");
                        selectedCell=cellList.get(selectedCell.cellnum+move);
                            
                        }else{
                        resultlbl.setText(resultlbl.getText()+"\n--------------\n"+selectedCell.cellnum+"->"+(selectedCell.cellnum+move)
                                 + "\n< Moved "+move+" cells forward >");
                        selectedCell=cellList.get(selectedCell.cellnum+move);}
                               act();
                    }
                    case 4 -> {
                         int move=(int) (Math.random()*6+1);
                         if (selectedCell.cellnum-move<=0) {
                             resultlbl.setText(resultlbl.getText()+"\n--------------\n"+"START<-"+selectedCell.cellnum
                                 + "\n< Moved "+selectedCell.cellnum+" cells backward >");
                          selectedCell=cellList.get(0);
                             
                            
                        } else {
                        
                         resultlbl.setText(resultlbl.getText()+"\n--------------\n"+(selectedCell.cellnum-move)+"<-"+selectedCell.cellnum
                                 + "\n< Moved "+move+" cells backward >");
                          selectedCell=cellList.get(selectedCell.cellnum-move);}
                        act();
                    }
    ...
    private void writeScores(){
        try {
            ...
            writer.write(username.getText()+", level"+difflevel+", "+score+"\n");
            writer.close();
